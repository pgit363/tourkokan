...
type StepTitleKey = 'step1Title' | 'step2Title' | 'step3Title' | 'step4Title'
type StepDescKey = 'step1Desc' | 'step2Desc' | 'step3Desc' | 'step4Desc'

const stepKeys: Array<{ num: string; titleKey: StepTitleKey; descKey: StepDescKey }> = [
      ...
      <div className="container">
        <div className="mb-14 text-center">
          <span className="inline-block rounded-full bg-primary-100 px-4 py-1.5 text-sm font-semibold text-primary-700 dark:bg-primary-900/40 dark:text-primary-300">
            {t.home.howSectionTag}
          </span>
          <h2 className="mt-4 text-4xl font-bold text-neutral-900 dark:text-white">{t.home.howSectionTitle}</h2>
          <p className="mx-auto mt-4 max-w-xl text-lg text-neutral-500 dark:text-neutral-400">
            {t.home.howSectionSubtitle}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
            {stepKeys.map((step, index) => (
              <div key={index} className="flex flex-col items-center text-center">
                ...
                <div className="mt-2 text-xs font-bold tracking-widest text-primary-500">{step.num}</div>
                <h3 className="mt-4 text-lg font-semibold text-neutral-900 dark:text-white">{t.home[step.titleKey]}</h3>
                <p className="mt-2 max-w-xs text-sm text-neutral-500 dark:text-neutral-400">{t.home[step.descKey]}</p>
              </div>
            ))}
          </div>
        </div>
